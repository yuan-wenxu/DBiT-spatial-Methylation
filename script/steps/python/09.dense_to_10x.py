...
def numeric_state(
    value: str, row_number: int, column_number: int
) -> tuple[bool, bool]:
    ...
    return True, not number.is_zero()


def should_write_mask(input_path: Path) -> bool:
    matrix_name = input_path.name.removesuffix(".gz").removesuffix(".csv")
    return matrix_name == "mean_shrunken_residuals"
...
def convert(
    ...
    write_mask = should_write_mask(input_path)
    ...
    mask_entries_path = None
    mask_tmp = None
    if write_mask:
        mask_entries_path = temporary_path(output_dir, ".mask.entries")
        mask_tmp = temporary_path(output_dir, ".mask.mtx.gz")
        temporary_files.extend((mask_entries_path, mask_tmp))
    ...
    try:
        with open_text(input_path) as input_handle:
            ...
            with ExitStack() as stack:
                entries_handle = stack.enter_context(
                    entries_path.open("w", encoding="utf-8", newline="")
                )
                barcodes_handle = stack.enter_context(
                    open_reproducible_gzip(barcodes_tmp)
                )
                mask_entries_handle = None
                if mask_entries_path is not None:
                    mask_entries_handle = stack.enter_context(
                        mask_entries_path.open(
                ...
                for csv_row_number, row in enumerate(reader, start=2):
                    ...
                    for feature_index, value in enumerate(row[1:], start=1):
                        observed, nonzero = numeric_state(
                            value, csv_row_number, feature_index + 1
                        )
                        if observed and mask_entries_handle is not None:
                            observed_count += 1
                            mask_entries_handle.write(
                                f"{feature_index} {barcode_count} 1\n"
                            )
                        if nonzero:
        ...
        if mask_tmp is not None and mask_entries_path is not None:
            with open_reproducible_gzip(mask_tmp) as mask_handle:
                mask_handle.write(
                    "%%MatrixMarket matrix coordinate integer general\n"
                )
                mask_handle.write(
                    "% observed-value mask generated by DBiT-spatial-Methylation\n"
                )
                mask_handle.write(
                    f"{len(features)} {barcode_count} {observed_count}\n"
                )
                with mask_entries_path.open(
                    "r", encoding="utf-8"
                ) as mask_entries_handle:
                    shutil.copyfileobj(mask_entries_handle, mask_handle)
        ...
        mask_output = output_dir / "mask.mtx.gz"
        if mask_tmp is not None:
            os.replace(mask_tmp, mask_output)
            result_observed_count = observed_count
        else:
            mask_output.unlink(missing_ok=True)
            result_observed_count = None
        return (
            len(features),
            barcode_count,
            nonzero_count,
            result_observed_count,
        )
    finally:
...
def main() -> None:
    args = parse_args()
    try:
        feature_count, barcode_count, nonzero_count, observed_count = convert(
            args.input,
            args.output,
            args.feature_type,
        )
    ...
    if observed_count is not None:
        observed_density = observed_count / (feature_count * barcode_count)
        print(f"[dbitm] mask: {args.output / 'mask.mtx.gz'}")
        print(
            f"[dbitm] observed: {observed_count}; "
            f"density: {observed_density:.6%}"
        )
