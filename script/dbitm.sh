...
Arguments:
  ...
  step           Pipeline step: fastp | barcode | align | spike-align | pool | mbias | call | spike-call | summary | all
...
ALL_STEPS=(fastp barcode align spike-align pool mbias call spike-call summary)
...
submit_all_hpc() {
    ...
    local pool_job_id mbias_job_id call_job_id spike_call_job_id summary_job_id step_name
    ...
    echo "[dbitm] job IDs: fastp=$fastp_job_id barcode=$barcode_job_id align=$align_job_id spike-align=$spike_align_job_id pool=$pool_job_id mbias=$mbias_job_id call=$call_job_id spike-call=$spike_call_job_id summary=$summary_job_id"
}
